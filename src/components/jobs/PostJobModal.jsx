    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Would you like to generate a document based on the job's scope and materials?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4 py-2">
          <button
            ...
            className="flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-primary/30 hover:border-primary hover:bg-primary/5 p-6 transition-all disabled:opacity-50"
          >
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
              <FileText className="w-6 h-6 text-primary" />
            </div>
            <div className="text-center">
              <p className="font-semibold text-sm">Generate Estimate</p>
              <p className="text-xs text-muted-foreground mt-0.5">Send for customer approval first</p>
            </div>
            {creating === "estimate" && isPending && (
              <div className="text-xs text-primary">Creating...</div>
            )}
          </button>
          ...
          <button
            ...
            className="flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-accent/30 hover:border-accent hover:bg-accent/5 p-6 transition-all disabled:opacity-50"
          >
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Ready to bill immediately</p>
            </div>
            {creating === "invoice" && isPending && (
              <div className="text-xs text-accent">Creating...</div>
            )}
          </button>
        </div>
